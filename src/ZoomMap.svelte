...
// Data
	...
	export function doHover(e) {
		...
		hover_dict = {"lad": hovered_lad, "lsoa": hovered_lsoa};	
	}
...
// FUNCTIONS (INCL. SCROLLER ACTIONS)
	...
	const actions = {
		...
		map01: () => {
			...
			map02: () => {
				fitFeatures(["E06000001","E06000002","E06000003","E06000004","E06000005","E06000047"], map, geojson);
				mapKey = "GVA2020";
				mapHighlighted = [];
				explore = false;
			},
                ...
                <MapSource
					...
					maxzoom={13}>
						{#if offset > 0.1 && index > 0 && map.getLayer('lad-fill')} 
							...
							/>
						{:else if id['map'] == 'map01' && index == 0 && map.getLayer('lsoa-fill')}
							{map.removeLayer('lsoa-fill')}
							{map.removeLayer('lsoa-line')}	
						{/if}
				/>
				</MapSource>
			</Map>
